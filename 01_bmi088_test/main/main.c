...
static esp_err_t bmi088_acc_init(void)
{
    ...
    ret |= bmi088_spi_write_reg(spi_acc_handle, BMI088_ACC_PWR_CTRL, 0x04); // 开启测量模式
    vTaskDelay(pdMS_TO_TICKS(100));

    ret |= bmi088_spi_write_reg(spi_acc_handle, BMI088_ACC_CONF, 0x8B);  // 800Hz输出速率 1 0x02 0x0B = 1 010 1011 =
    ret |= bmi088_spi_write_reg(spi_acc_handle, BMI088_ACC_RANGE, 0x01); // ±6g量程

    ret |= bmi088_spi_write_reg(spi_acc_handle, BMI088_ACC_INT_EN_1, 0x01);
    vTaskDelay(pdMS_TO_TICKS(5));

    uint8_t pwr_ctrl = 0, acc_conf = 0;

    bmi088_spi_read_reg(spi_acc_handle, BMI088_ACC_CONF, &acc_conf);
...
static esp_err_t bmi088_gyro_init(void)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(10));
    // 读取芯片ID
    ret = bmi088_spi_read_reg(spi_gyro_handle, BMI088_GYRO_CHIP_ID, &chip_id);
    if (ret != ESP_OK || chip_id != 0x0F) {
        ESP_LOGE(TAG, "GYRO chip ID error: 0x%02X", chip_id);
        return ESP_FAIL;
    }

    // 软复位陀螺仪
    ret = bmi088_spi_write_reg(spi_gyro_handle, 0x14, 0xB6);
    vTaskDelay(pdMS_TO_TICKS(20));

    // 低噪声配置
    ret = bmi088_spi_write_reg(spi_gyro_handle, BMI088_GYRO_BANDWITH, 0x05); // 100Hz输出+低通滤波
    ret |= bmi088_spi_write_reg(spi_gyro_handle, BMI088_GYRO_RANGE, 0x01);   // ±500°/s量程
...
esp_err_t spi_setup(void)
{
    ...
    // 初始化SPI总线（禁用DMA）
    ESP_ERROR_CHECK(spi_bus_initialize(BMI088_HOST, &bmi088_spi_bus, SPI_DMA_DISABLED));
 ...
 * @param arg 任务参数（未使用）
 */
void bmi088_test_task(void *arg)
{
...
void app_main(void)
{
    printf("BMI088 SPI Driver Start!\n");

    spi_setup();
    // 创建BMI088测试任务（栈大小10240，优先级5）
    xTaskCreate(bmi088_test_task, "bmi088_test_task", 10240, NULL, 5, NULL);
